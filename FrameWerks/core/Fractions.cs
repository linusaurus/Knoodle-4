...
namespace Weaselware.FractionalNumbers
{
   /// <summary>
   ...
   public struct Fraction : IComparable, IConvertible, IFormattable
   {
      ...
      /// </remarks>
      public static Fraction ToFraction(Decimal convertedDecimal)
      {
         unchecked
         {
      ...
      public static string DecimalToFraction(string dec)
      {
         string raw = dec;

         try
         {
            string end = raw.Substring(raw.IndexOf("."));
            string start = Decimal.Truncate(Convert.ToDecimal(raw)).ToString();

            decimal dc = Convert.ToDecimal(end);
            Fraction frac = FractionalNumbers.Fraction.ToFraction(dc);
            if (frac.ToString().Equals("0/1"))
            {
               return start;
            }
            else
